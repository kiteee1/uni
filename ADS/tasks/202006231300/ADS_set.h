...
public:
    ...
    const_iterator z()
    {
        ...
        return Iterator(temp.get_current(), 0, true);
    }
};
...
public:
    ...
    explicit Iterator(DataNode *cur, size_type pos, bool special = false) : current_node{cur}, position{pos}, special_mode{special}, special_counter{0} {
    ...
    Iterator &operator++()
    {
        if (current_node)
        {
            if (special_mode)
            {
                if (special_counter % 2 == 0)
                {
                    // Stay on the same element
                }
                else
                {
                    // Move to the next element, skipping one element
