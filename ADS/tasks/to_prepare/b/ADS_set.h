...
public:
    ...
    const_iterator y() const
    {
        ...
        if (total_elements == 1)
        {
            return const_iterator(begin().get_current_node(), 0, false);
        }
        // Find the last node and position
        ...
        return const_iterator(last_node, last_position, true, total_elements, begin().get_current_node(), last_node, last_position);
    }
};
...
public:
    ...
    explicit Iterator(DataNode *cur, size_type pos, bool special = false, size_type total = 0, DataNode *first = nullptr, DataNode *last = nullptr, size_type last_pos = 0)
        : current_node{cur}, position{pos}, special_mode{special}, special_counter{0}, total_elements{total}, first_node{first}, last_node{last}, last_position{last_pos} {}
    ...
    Iterator &operator++()
    {
        if (current_node)
        {
            ...
            if (special_mode)
            {
                ...
                else if (special_counter > total_elements - 1)
                {
                    current_node = nullptr; // This will make the iterator equal to end()
                    position = 0;
                }
                else
                {
