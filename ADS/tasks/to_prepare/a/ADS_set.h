...
public:
    ...
    const_iterator z() const
    {
        ...
        return Iterator(temp.get_current(), 0, true, total_elements, temp.get_current());
    }
};
...
public:
    ...
    Iterator &operator++()
    {
        ...
        {
            ++special_counter;
            if (special_mode && special_counter == total_elements - 1)
            {
                current_node = first_node;
                position = 0;
            }
            else if (special_mode && special_counter > total_elements - 1)
            {
                current_node = nullptr; // Setting current_node to nullptr makes the iterator equal to end()
                position = 0;
            }
            else
            {
