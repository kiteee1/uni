...
public:
    ...
    const_iterator z() const
    {
        ...
        return const_iterator(begin().get_current_node(), begin().get_position(), true, total_elements, begin().get_current_node());
    }
};
...
public:
    ...
    explicit Iterator(DataNode *cur, size_type pos, bool special = false, size_type total = 0, DataNode *first = nullptr)
        : current_node{cur}, position{pos}, special_mode{special}, special_counter{0}, total_elements{total}, first_node{first}
    {
    }
    ...
    Iterator &operator++()
    {
        if (current_node)
        {
            ...
            if (special_mode)
            {
                ...
                size_type remaining_elements = total_elements - special_counter;

                // Skip every third element, but not the last one
                if (special_counter % 3 == 2 && remaining_elements > 1)
                {
                    ...
                    ++special_counter; // Additional increment to skip this element
                }
