...
public:
    ...
    {
        DeletionStatus status{root->delete_key(key)};
        switch (status)
        {
        case DeletionStatus::SUCCESS:
            --sz;
            return 1;
        case DeletionStatus::NOT_FOUND:
            return 0;
        case DeletionStatus::NEEDS_MERGE:
            if (root->node_size == 0)
            {
                if (root->type() == NodeType::KEY_NODE)
                {
                    std::pair<key_type *, link *> elems = root->retrieve_all();
                    delete root;
                    root = elems.second[0];
                    delete[] elems.first;
                    delete[] elems.second;
    ...
    {
        size_type total_elements = size();
        auto it = begin();

        DataNode *last_node = it.get_current_node();
        size_type last_position = it.get_position();

        while (it != end())
        {
            last_node = it.get_current_node();
            last_position = it.get_position();
            ++it;
        }
        return const_iterator(begin().get_current_node(), begin().get_position(), true, total_elements, last_node, last_position);
    }
};
...
private:
    ...
    size_type position;
    bool special_mode;
    size_type special_counter;
    size_type total_elements;
    size_type start_skip;
    DataNode *last_node;
    size_type last_position;

public:
    Iterator() : current_node{nullptr}, position{0} {}
    explicit Iterator(DataNode *cur, size_type pos, bool special = false, size_type total = 0, DataNode *last_node = nullptr, size_type last_position = 0)
        : current_node{cur}, position{pos}, special_mode{special}, special_counter{0}, total_elements{total}, last_node{last_node}, last_position{last_position}
    {
        start_skip = total_elements / 2;
    }

    DataNode *get_current_node() const
    {
        return current_node;
    }
    ...
    Iterator &operator++()
    {
        if (current_node)
        {
            ...
            if (special_mode)
            {
                if (special_counter == start_skip)
                {
                    current_node = last_node;
                    position = last_position; // This will make the iterator equal to end()
                }
                else if (special_counter > start_skip)
                {
                    current_node = nullptr;
                    position = 0; // This will make the iterator equal to end()
                }
                else
                {
...
struct ADS_set<Key, N>::NodeBase
{
    ...
    virtual void remove_at_position(size_type position)
    {
    ...
    virtual DeletionStatus delete_key(const key_type &elem) = 0;
    virtual std::pair<key_type *, link *> retrieve_all() = 0;
    virtual void set_merge(const key_type &pulled_down) = 0;
    virtual void do_merge(link neighbour) = 0;
...
struct ADS_set<Key, N>::DataNode : public NodeBase
{
    ...
    DeletionStatus delete_key(const key_type &elem) override
    {
        int pos{this->find_position(elem)};
        if (pos < 0)
            return DeletionStatus::NOT_FOUND;
        this->remove_at_position(static_cast<size_type>(pos));

        return this->node_size >= N ? DeletionStatus::SUCCESS : DeletionStatus::NEEDS_MERGE;
    }

    std::pair<key_type *, link *> retrieve_all() override
    {
    ...
    void set_merge(const key_type &) override {}

    void do_merge(link neighbour) override
    {
        std::pair<key_type *, link *> elems{neighbour->retrieve_all()};
...
struct ADS_set<Key, N>::KeyNode : public NodeBase
{
    ...
    void remove_at_position(size_type position) override
    {
        delete child_nodes[position + 1];
        for (size_type i{position}; i < this->node_size - 1; ++i)
        {
            this->values[i] = this->values[i + 1];
    ...
    DeletionStatus delete_key(const key_type &elem) override
    {
        ...
        DeletionStatus status{child_nodes[childpos]->delete_key(elem)};
        if (status == DeletionStatus::NEEDS_MERGE)
        {
            if constexpr (N > 1)
            {
                ...
                if (totalsize > NodeBase::MaxElements)
                {
                    ...
                    if (split_at < left->node_size)
                    {
                        ...
                        splitres.first->set_merge(id);
                        splitres.first->do_merge(right);
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    this->remove_at_position(childpos - 1);
                }
            }
            else
            {
                ...
                child_nodes[childpos - 1]->set_merge(this->values[childpos - 1]);
                child_nodes[childpos - 1]->do_merge(child_nodes[childpos]);
    ...
    void set_merge(const key_type &pulled_down) override
    {
