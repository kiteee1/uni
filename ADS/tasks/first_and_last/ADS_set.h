...
public:
    ...
    const_iterator z() const
    {
        ...
        return const_iterator(begin().get_current_node(), 0, true, last_node, last_position);
    }
};
...
public:
    ...
    explicit Iterator(DataNode *cur, size_type pos, bool special = false, DataNode *last_node = nullptr, size_type last_position = 0)
        : current_node{cur}, position{pos}, special{special}, last_node{last_node}, last_position{last_position}, counter{0} {}
    ...
    Iterator &operator++()
    {
        ...
        {
            ++counter;
            if (special && counter == 1)
            {
                current_node = last_node;
                position = last_position;
            }
            else if (special && counter > 1)
            {
