...
public:
    ...
    {
        return const_iterator(begin().get_current_node(), 0, true);
    }
};
...
private:
    ...
    size_type duplicate_counter;

public:
    Iterator() : current_node{nullptr}, position{0} {}
    explicit Iterator(DataNode *cur, size_type pos, bool special = false) : current_node{cur}, position{pos}, special{special}, special_counter{0}, duplicate_counter{0}
    {
        if (special)
        {
            ++(*this);
    ...
    Iterator &operator++()
    {
        if (current_node)
        {
            if (special)
            {
                ...
                if (duplicate_counter == 2) // Move to the next element after duplicating once
                {
                    duplicate_counter = 0;
                    do
                    {
                    ...
                    } while (current_node && special_counter % 2 == 0); // Skip even indices
                }
            }
            else
            {
