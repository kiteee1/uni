...
Hero::Hero(std::string name, Hero_Class hero_class, Hero_Species hero_species, unsigned max_hp, const std::map<std::string, unsigned> &abilities) : name{name}, hero_class{hero_class}, hero_species{hero_species}, max_hp{max_hp}, abilities{abilities}, in_campaign{false}
{
...
void Hero::join_campaign(const Campaign &campaign)
{
    in_campaign = true;
    campaign_logbook.push_back(campaign);
}
...
void Hero::add_campaign_to_log(const Campaign &campaign)
{
    campaign_logbook.push_back(campaign);
}

void Hero::print_logbook(std::ostream &o) const
{
    o << name << "'s Logbook: ";
    for (const auto &entry : campaign_logbook)
    {
        o << "Campaign: " << entry.get_name() << ", Level: " << level << "\n";
    }
}
