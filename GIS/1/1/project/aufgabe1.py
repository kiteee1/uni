...
def get_improved_heuristic(additional_info, goal_city):
    ...
    for city_key, info in additional_info.items():
        ...
        heuristic_value = float(np.sqrt(line_of_sight**2 + altitude_difference**2))
        heuristic[city_key] = heuristic_value
    return heuristic
...
def a_star_search(graph, start, goal, heuristic):
    ...
    start_key = f'city_{start}'
    frontier.push(heuristic.get(start_key, 0.0), 0, start, [start])
    explored = {}
    expanded_nodes = 0

    while not frontier.is_empty():
        estimated_total_cost, cost_so_far, current_city, path = frontier.pop()

        if current_city in explored and cost_so_far > explored[current_city]:
            continue
        ...
        for neighbor, neighbor_cost in graph.get(current_city, {}).items():
            ...
            if neighbor in explored and total_cost_so_far >= explored[neighbor]:
                continue
